...
export const useAuthStore = create<AuthState>()(
  persist(
    ...
    {
      name: 'everly-auth-session-v1',
      partialize: (state) => ({ currentUser: state.currentUser }),
    }
  )
)

// hydrate users from server on load (in case we persisted only the session)
void (async function loadUsersFromServer() {
  try {
    const state = await fetchServerState()
    if (Array.isArray(state.users) && state.users.length > 0) {
      useAuthStore.setState({ users: state.users })
    }
  } catch {
    // ignore
  }
