    ...
    <div className="min-h-dvh bg-gradient-to-b from-white to-brand-50 dark:from-black dark:to-gray-950">
      <header className="relative overflow-hidden border-b border-brand-100 dark:border-gray-800">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-brand-700 to-pink-600 dark:from-brand-300 dark:to-pink-300 bg-clip-text text-transparent">Today's Chores</h1>
            <p className="mt-2 text-sm text-gray-700 dark:text-gray-200">These reset every midnight. Check them off as you go.</p>
          </div>
          {currentUser && (
            <button
              onClick={() => setShowForm((s) => !s)}
              className={`inline-flex items-center gap-2 rounded-full px-4 py-2 ${currentUser ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/30 hover:bg-brand-600 hover:shadow-brand-600/30' : 'bg-gray-300 text-gray-600 cursor-not-allowed'}`}
            >
              <PlusCircle className="size-5" /> {showForm ? 'Hide form' : 'Add chore'}
            </button>
          )}
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        <section className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Your chores</h2>
